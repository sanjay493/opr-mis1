...
Called after extraction when a plant has per-furnace techno rows in techno_actuals
...
Aggregation rules:
  ...
  - Blast Temperature:                      simple average
...
Design principle:
  Source extractors write raw per-furnace values.
  This module writes shop-aggregate rows (source_priority=4).
  Directly-extracted shop averages (priority 5) always win on conflict.
"""

import sqlite3
from typing import Dict, List, Optional, Tuple

# IRON_MAKING param_name → MAJOR param_name (all canonical)
_SECTION_TO_MAJOR = {
    "Coke Rate":       "Coke Rate",
...
# ── Parameters that use weighted average (canonical names) ──────────────────
_WEIGHTED_SECTIONS = frozenset({
    "Coke Rate", "Nut Coke Rate", "CDI Rate", "Fuel Rate",
...
def _load_plant_furnaces(conn: sqlite3.Connection, plant: str
    ...
    """Load per-furnace BF units from plant_units (or _old_plant_units after migration).
    Returns ([(display_label, prod_item_or_None), ...], shop_display_label).
    """
    # Try new plant_units first, fall back to _old_plant_units after migration
    for tbl in ("plant_units", "_old_plant_units"):
        cur = conn.execute(
            f"SELECT name FROM sqlite_master WHERE type='table' AND name=?", (tbl,)
        )
        if cur.fetchone():
            cur = conn.execute(
                f"SELECT unit_name, display_label, is_shop FROM {tbl} "
        ...
        for section in sections:
            ...
            for fce_label, prod_item in furnaces:
                cur = conn.execute(
                    ...
                    "WHERE g.group_code='IRON_MAKING' AND p.param_name=? AND p.row_label=?",
                    (section, fce_label),
                )
                row = cur.fetchone()
                if not row:
            ...
            # Find unit from an existing furnace row
            cur3 = conn.execute(
                "SELECT p.unit FROM techno_param p "
                "JOIN techno_param_group g ON p.param_id=g.param_id "
                "WHERE g.group_code='IRON_MAKING' AND p.param_name=? AND p.row_label!=? LIMIT 1",
                (section, shop_label),
            )
            ...
            sort_order_map = {
                "Coke Rate": 48, "Nut Coke Rate": 58, "CDI Rate": 10,
            ...
            save_techno_monthly(param_id, report_month, round(agg, 4), None,
                                source_priority=4)
            # Mirror to MAJOR row at priority 4
            major_section = _SECTION_TO_MAJOR.get(section)
            major_row_label = _SHOP_LABEL_TO_MAJOR_ROW.get(shop_label)
            if major_section and major_row_label:
                cur_maj = conn.execute(
                    "SELECT p.param_id FROM techno_param p "
                    "JOIN techno_param_group g ON p.param_id=g.param_id "
                    "WHERE g.group_code='MAJOR' AND p.param_name=? AND p.row_label=?",
                    (major_section, major_row_label),
                )
...
def compute_fuel_rate_fallback(conn: sqlite3.Connection, report_month: str) -> int:
    ...
    of Coke Rate + Nut Coke Rate + CDI Rate, write Fuel Rate = sum of the three at
    source_priority=3.  Returns count of rows written.
    """
    from db import get_or_create_techno_param, save_techno_monthly

    cur = conn.execute(
        "SELECT DISTINCT p.row_label FROM techno_param p "
        "JOIN techno_param_group g ON p.param_id=g.param_id "
        "WHERE g.group_code='IRON_MAKING' AND p.param_name='Coke Rate'"
    )
    ...
    for label in coke_labels:
        def _val(param_name):
            cur2 = conn.execute(
                "SELECT a.actual FROM techno_actuals a "
                "JOIN techno_param p ON p.param_id=a.param_id "
                "JOIN techno_param_group g ON p.param_id=g.param_id "
                "WHERE g.group_code='IRON_MAKING' AND p.param_name=? "
                "AND p.row_label=? AND a.report_month=?",
                (param_name, label, report_month),
            )
            row = cur2.fetchone()
            return row[0] if row else None
...
def recompute_all_months(conn: sqlite3.Connection,
    ...
    cur = conn.execute("SELECT DISTINCT report_month FROM techno_actuals ORDER BY report_month")
