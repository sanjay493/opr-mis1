...
def _fetch_group(cur, month, plant, product):
    cur.execute("""
        SELECT quality_grade, order_qty, actual_despatch
...
def _fetch_all_grades(cur, all_months, plant, product):
    """Union of quality grades that have any record in the given months,
    ordered by sort_order. Covers current month, YTD, CPLY and CPLY-YTD."""
    ph = ",".join("?" * len(all_months))
    cur.execute(f"""
        SELECT quality_grade, MIN(sort_order)
        FROM special_steel_orders
        WHERE report_month IN ({ph}) AND plant_name=? AND product=?
        GROUP BY quality_grade
        ORDER BY MIN(sort_order), quality_grade
    """, (*all_months, plant, product))
    return [r[0] for r in cur.fetchall()]

def _fetch_cply(cur, cply_month, plant, product):
    cur.execute("""
        SELECT quality_grade, actual_despatch
        FROM special_steel_orders
        WHERE report_month=? AND plant_name=? AND product=?
...
def _fetch_group_ytd(cur, ytd_months, plant, product):
    """quality_grade → (sum_order_qty, sum_actual_despatch) across YTD months."""
...
def _fetch_cply_ytd(cur, cply_ytd_months, plant, product):
    """quality_grade → sum_actual_despatch across CPLY YTD months."""
    ph = ",".join("?" * len(cply_ytd_months))
    cur.execute(f"""
        SELECT quality_grade, COALESCE(SUM(actual_despatch),0)
        FROM special_steel_orders
        WHERE report_month IN ({ph}) AND plant_name=? AND product=?
        GROUP BY quality_grade
...
def _build_group(cur, month, cply_month, ytd_months, cply_ytd_months,
    ...
    cur_map  = {qg: (o, a) for qg, o, a in _fetch_group(cur, month, plant, product_group)}
    ...
    for qg in all_grades:
        o, a   = cur_map.get(qg, (0, 0))
        o, a   = o or 0, a or 0
        c      = cply_map.get(qg)
        co, ca = ytd_map.get(qg, (0, 0))
        cc     = cytd_map.get(qg)
        ...
        rows.append(_grade(qg, o, a, c, co, ca, cc))
...
def _gen_isp(cur, month, cply_month, ytd_months, cply_ytd_months):
    """ISP: mill-wise, quality_grade stored as 'TOTAL' per row."""
    mills   = ["US Mill", "Semis", "WRM", "Bar Mill"]
...
_PLANT_TITLES = {
    ...
    "ISP": "SPECIAL STEEL PERFORMANCE FOR ISP",
}
...
def generate_special_steel_plant(report_month: str, plant: str) -> dict:
    ...
    try:
        ...
        ss_cur  = _get_prod(cur, report_month, plant, "Saleable Steel")
        ss_cply = _get_prod(cur, cply_month,   plant, "Saleable Steel")
        ss_cum  = _get_prod_ytd(cur, ytd_months,      plant, "Saleable Steel")
        ss_ccum = _get_prod_ytd(cur, cply_ytd_months, plant, "Saleable Steel")
...
def generate_special_steel_sail(report_month: str) -> dict:
    ...
    try:
        ...
        ss_cur  = sum(v for p in plants for v in [_get_prod(cur, report_month, p, "Saleable Steel")] if v)
        ss_cply = sum(v for p in plants for v in [_get_prod(cur, cply_month,   p, "Saleable Steel")] if v)
        ss_cum  = sum(v for p in plants for v in [_get_prod_ytd(cur, ytd_months,      p, "Saleable Steel")] if v)
        ss_ccum = sum(v for p in plants for v in [_get_prod_ytd(cur, cply_ytd_months, p, "Saleable Steel")] if v)
