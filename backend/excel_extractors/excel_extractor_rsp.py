...
P18_TECHNO_ITEMS = [
    ...
    ("SP-3 Sinter Productivity",   ["sp 3 sinter productivity"],                             80),
    # Furnace-wise productivity BEFORE generic so exact-scan claims them first
    ("BF Productivity BF-1",       ["productivity bf 1"],                                     96),
    ("BF Productivity BF-4",       ["productivity bf 4"],                                     97),
    ("BF Productivity BF-5",       ["productivity bf 5"],                                     98),
    # "Productivity - (Shop)" normalises to "productivity shop"
    ("BF Productivity",            ["productivity shop", "bf productivity"],                99),
    # Furnace-wise coke rate entries BEFORE generic so exact-scan claims them first
    ("Coke Rate BF-1",             ["coke rate bf 1", "coke rate bf#1"],                      100),
    ("Coke Rate BF-4",             ["coke rate bf 4", "coke rate bf#4"],                      101),
    ("Coke Rate BF-5",             ["coke rate bf 5", "coke rate bf#5"],                      102),
    ("Coke Rate",                  ["coke rate"],                                            103),
    ("CDI BF-1",                   ["cdi bf 1", "pci bf 1"],                                104),
    ("CDI BF-4",                   ["cdi bf 4", "pci bf 4"],                                  105),
    ("CDI BF-5",                   ["cdi bf 5", "pci bf 5"],                                106),
    ("CDI",                        ["cdi", "pci"],                                           107),
    ("Coal to Hot metal ratio",    ["coal to hot metal ratio", "coal to hm ratio"],          112),
    ("Nut Coke BF-1",              ["nut coke bf 1", "nut coke bf#1", "n c bf 1"],            108),
    ("Nut Coke BF-4",              ["nut coke bf 4", "nut coke bf#4", "n c bf 4"],            109),
    ("Nut Coke BF-5",              ["nut coke bf 5", "nut coke bf#5", "n c bf 5"],            110),
    ("Nut Coke Rate",              ["nut coke rate", "nut coke"],                            111),
    ...
    ("Hot Blast Temp BF-5",        ["hot blast temp bf 5", "hot blast temperature bf 5",
                                    "hbt bf 5"],                                               139),
    ("Hot Blast Temp",             ["hot blast temp", "hot blast temperature", "hbt"],         140),
    ("Si% in HM BF-1",             ["si% in hm bf 1", "si in hm bf 1", "silicon in hm bf 1"],142),
    ("Si% in HM BF-4",             ["si% in hm bf 4", "si in hm bf 4", "silicon in hm bf 4"],143),
    ("Si% in HM BF-5",             ["si% in hm bf 5", "si in hm bf 5", "silicon in hm bf 5"],144),
    ("Si% in HM",                  ["si% in hm", "si in hm", "silicon% in hm",
                                    "silicon in hm"],                                          145),
    ("O2 Enrichment BF-1",         ["o2 enrichment bf 1", "oxygen enrichment bf 1"],           132),
    ("O2 Enrichment BF-4",         ["o2 enrichment bf 4", "oxygen enrichment bf 4"],           133),
    ("O2 Enrichment BF-5",         ["o2 enrichment bf 5", "oxygen enrichment bf 5"],           134),
    ("O2 Enrichment",              ["o2 enrichment", "oxygen enrichment", "o 2 enrichment"],   135),
    ("Sinter% in Burden",          ["sinter in burden", "sinter% in burden"],               123),
    ("Pellet% in Burden",          ["pellet in burden", "pellet% in burden"],               124),
    ("Fuel Rate",                  ["fuel rate"],                                            155),
    ("SMS-1 HM consumption per ton of crude steel",
                                  ["sms 1 hm consumption per ton", "sms 1 hm cons per ton"],162),
    ("SMS-1 Scrap consumption per ton of crude steel",
                                  ["sms 1 scrap consumption per ton", "sms 1 scrap cons"],  163),

    ("SMS-1 TMI consumption per ton of crude steel",
                                  ["sms 1 TMI consumption per ton", "sms 1 TMI cons"],  161),               
    ...
    ("SMS-2 HM consumption per ton of crude steel",
                                  ["sms 2 hm consumption per ton", "sms 2 hm cons per ton"],189),
    ("SMS-2 Scrap consumption per ton of crude steel",
                                  ["sms 2 scrap consumption per ton", "sms 2 scrap cons"],  190),
    ("SMS-2 TMI consumption per ton of crude steel",
                                  ["sms 2 tmi consumption per ton", "sms 2 tmi cons"],  188),
    
    
    ("SMS-2 Avg heat wt",          ["sms 2 avg heat wt"],                                   203),
    ("SMS-1 lining life",          ["sms 1 lining life", "avg lining life nos"],           176),
    ("SMS-2 lining life",          ["sms 2 lining life"],                                   205),
    ("SMS-2 Avg Blows per day",    ["blow sms2", "blow sms 2", "sms 2 avg blows per day"],  212),
    ("Oven heat Consumption per ton of Dry coke Input",
                                  ["oven heat consumption", "oven heat cons per ton"],       304),
...
SHOPS = [
    ("Plate Mill", ["plate mill"], [
        ...
        ("Mill Utilisation",    ["util on avail", "mill utilisation", "mill utilization"],
                                                          "%Avl.",   5, 224),
        ("Rolling Rate",        ["rolling rate"],         "T/Hr.",   6, 225),
    ...
    ("HSM-II", ["hsm ii", "hsm 2", "hot strip mill ii", "hot strip mill 2"], [
        ...
        ("Average Slab Weight",    ["average slab weight"],
                                                          "Tons",   11, 253),
        ("Mill Availability",      ["mill availability"], "%",      12, 254),
        ...
        ("Rolling Rate -HR Coils", ["rolling rate hr coils", "rolling rate"],
                                                          "T/Hr.",  14, 256),
        ("Specific Heat",          ["specific heat"],     "M.Cal/T",15, 312),
        ("Specific Power",         ["specific power"],    "Kwh/T",  16, 330),
    ]),
    ("New Plate Mill", ["new plate mill", "npm"], [
        ("Yield - Primes",      ["yield primes", "yield prime"],
                                                          "%",      20, 233),
        ("Yield - Total",       ["yield total"],          "%",      21, 234),
        ("Average Slab Weight", ["average slab weight"],  "Tons",   22, 235),
        ("Mill Availability",   ["mill availability"],    "%",      23, 236),
        # file label: "Util. on Avail. Time"
        ("Mill Utilisation",    ["util on avail", "mill utilisation", "mill utilization"],
                                                          "%Avl.",  24, 237),
        ("Rolling Rate",        ["rolling rate"],         "T/Hr.",  25, 238),
        ("Specific Heat",       ["specific heat"],        "M.Cal/T",26, 310),
        ("Specific Power",      ["specific power"],       "Kwh/T",  27, 328),
    ]),
    ...
    ("ERW Pipe Plant", ["erw pipe plant", "erw pipe"], [
        ("Yield from HR Coils", ["yield from hr coils"],  "%",     40, 260),
        ("Mill Availability",   ["mill availability"],     "%",     41, 262),
        # file label: "Utili.on Available Time"
        ("Mill Utilisation",    ["utili on available", "utili on avail",
                                 "mill utilisation", "mill utilization"],
                                                          "%Avl.", 42, 263),
        ("Rolling Rate",        ["rolling rate"],          "T/Hr.", 43, 264),
    ]),
    ("SW Pipe Plant", ["sw pipe plant", "sw pipe", "spiral weld pipe"], [
        ("Yield from HR Coils", ["yield from hr coils"],  "%",     50, 267),
        ("Mill Availability",   ["mill availability"],     "%",     51, 269),
        # file label: "Utili. on Avail. Time"
        ("Mill Utilisation",    ["utili on avail", "utili on available",
                                 "mill utilisation", "mill utilization"],
                                                          "%Avl.", 52, 270),
        ("Rolling Rate",        ["rolling rate"],          "T/Hr.", 53, 271),
    ]),
]
