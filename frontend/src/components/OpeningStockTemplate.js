...
// ── style tokens ──────────────────────────────────────────────────────────────
const CELL = { padding: '1px 4px', border: '1px solid #94a3b8', lineHeight: 1.15, fontSize: '0.66rem' };
const NUM  = { ...CELL, textAlign: 'right' };
const LBL  = { ...CELL, textAlign: 'left' };
const TH   = {
  backgroundColor: '#1e3a5f', color: '#fff', padding: '2px 3px',
  textAlign: 'center', verticalAlign: 'middle',
  border: '1px solid #334155', fontSize: '0.64rem', lineHeight: 1.2, fontWeight: 600,
};
const SECTION_CELL = {
  ...CELL, textAlign: 'center', verticalAlign: 'middle',
  fontWeight: 700, fontSize: '0.6rem', backgroundColor: '#e2e8f0',
  ...
  return (
    <div style={{ padding: '6px', fontFamily: 'Arial, sans-serif' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', border: '2px solid #1e293b',
                      tableLayout: 'fixed', fontSize: '0.66rem' }}>
        <colgroup>
                ...
                <tr key={`${si}-${ri}`} style={{ backgroundColor: bg, fontWeight: fw }}>
                  ...
                  {ri === 0 && (
                    <td rowSpan={sec.rows.length}
                        style={{ ...SECTION_CELL, fontSize: '0.56rem' }}>
                      {sec.code}
                    </td>
                  )}
                  <td style={{ ...LBL, fontSize: '0.6rem' }}>{row.sub}</td>
