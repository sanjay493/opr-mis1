...
import {
  API_BASE, MONTH_NAMES_FULL, MONTH_NUM, YEARS, getDefaultPeriod, monthLabel, fmtNum,
  cell, selStyle, th, pillStyle, SegmentedToggle, ErrorBox, EmptyState, TabIntro,
  ExportButtons, downloadFile,
} from './shared';
...
export default function TechnoCustomView() {
  ...
  // A generated table no longer matches once plants/params/periods change —
  // hide it so the on-screen table and the Excel/PDF export (which is built
  // from the current selection) can't disagree.
  const periodInputsKey = JSON.stringify([selectedPlants, selectedParams, periods]);
  const [periodDataKey, setPeriodDataKey] = useState(null);
  const periodDataCurrent = periodDataKey === periodInputsKey ? periodData : null;
  ...
  const fetchPeriodData = () => {
    ...
    fetch(`${API_BASE}/api/techno-custom-period`, {
      ...
      .then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); })
      .then((d) => { setPeriodData(d); setPeriodDataKey(periodInputsKey); })
  ...
  const saveFile = async (url, body, filename) => {
    try {
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <SegmentedToggle
          options={[['standard', 'Standard (Page 27 style)'], ['custom', 'Custom Period']]}
      ...
      {mode === 'standard' && (
        <>
          ...
          {!loadingStandard && filteredSections.length === 0 && (
            <EmptyState>No techno data for the selected plants/parameters in {reportMonth}.</EmptyState>
          )}
      ...
      {mode === 'custom' && (
        <>
          ...
          {periodDataCurrent && periodDataCurrent.sections?.length > 0 && (
            <div style={{
              border: '1px solid #dadce0', borderRadius: '8px',
              ...
              <table style={{ borderCollapse: 'separate', borderSpacing: 0, width: '100%' }}>
                ...
                <tbody>
                  {periodDataCurrent.sections.map((sec) => (
                    <React.Fragment key={sec.parameter}>
                      <tr>
                        <td colSpan={1 + periodDataCurrent.periods.length} style={{
                          ...cell, backgroundColor: '#1a73e8', color: '#ffffff',
                          fontWeight: 800, fontSize: '11pt', letterSpacing: '0.02em',
                        }}>
          ...
          {periodDataCurrent && periodDataCurrent.sections?.length === 0 && (
            <EmptyState>No data for the selected plants/parameters in these periods.</EmptyState>
          )}
        </>
      )}
    </div>
