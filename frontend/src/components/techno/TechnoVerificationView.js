...
import {
  API_BASE, FY_MONTHS, MONTH_NUM, YEARS, getDefaultPeriod, fmtNum as fmtNum3,
  cell, selStyle, th, ErrorBox, EmptyState, TabIntro, ExportButtons, downloadFile,
} from './shared';

const fmtNum = (v) => fmtNum3(v, 4);
...
export default function TechnoVerificationView() {
  ...
  const handleDownload = async (kind) => {
    ...
    try {
      await downloadFile(
        `${API_BASE}/api/techno-major-verification/${kind}?month=${reportMonth}`,
        undefined,
        `Techno_Verification_${reportMonth}.${kind === 'excel' ? 'xlsx' : 'pdf'}`
      );
    } catch (e) {
    ...
    <div>
      ...
      {!error && data && (
        data.sections?.length ? (
          ...
          <EmptyState>No techno data for {reportMonth}.</EmptyState>
        )
      )}
    </div>
  );
