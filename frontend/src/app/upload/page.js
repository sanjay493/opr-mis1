            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '25px' }}>
              <div>
                ...
                <ul style={{ fontSize: '8.5pt', color: '#202124', lineHeight: '1.6', margin: 0, paddingLeft: '15px' }}>
                  ...
                  <li><strong>BSL — Production of Main Products (.pdf):</strong> The plant&apos;s month-end PDF bundle (e.g. <em>Rev &lt;Mon&gt;&lt;YY&gt; (n).pdf</em>) — auto-detected by its <strong>&quot;PRODUCTION OF MAIN PRODUCTS&quot;</strong> page title, distinguishing it from the BF Performance PDF above. Reads the same production items as the DPR path (Oven Pushing, Sinter, Hot Metal, Pig Iron, SMS-1 CCM-1, SMS-2 CCM-1&amp;2, Crude Steel, HSM HR Coil (total &amp; saleable), HSM HR Plate, HR Sheet, CR I/II CR(Coil) Sale, CR Sheets, CR III CR(Coil) Sale, GPC3, GP/GC, CR(1&amp;2) Total Saleable, CR III Total Saleable, CRSALE, Saleable Steel, Finished Steel, Saleable Semis) from the finalised monthly figures — Finished Steel and Saleable Semis come from the report&apos;s page 3 breakup table. Month auto-detected from the page header (e.g. &quot;JUNE 2026&quot;).</li>
                  ...
                  <li><strong>ASP — REP*.pdf</strong> (Preview &amp; Insert, plant: ASP): OMI Daily Performance Summary Report — month <strong>auto-detected</strong> from its own &quot;...REPORT FOR DD/MM/YYYY&quot; line. Extracts <strong>Total Crude Steel, Total Caster, Ingot Steel, Saleable Steel, Saleable Steel Despatch</strong> (&quot;PLANT DESPATCH&quot; row — each row&apos;s cumulative-actual column, matched by exact label — not a generic keyword search, to avoid the page&apos;s overlapping equipment-delay table and historical-actuals rows) plus <strong>Closing Stock</strong> (&quot;TOTAL PLANT ST.&quot;).</li>
                  <li><strong>ASP — FL*.pdf</strong> (Preview &amp; Insert, plant: ASP): FLASH monthly report — report month <strong>auto-detected</strong> from the table&apos;s own &quot;DETAILS&quot; header row. Extracts <strong>ING</strong> (Production for Finishing) → <strong>Ingot Semis</strong>, <strong>BILLETS, BARS, PL MILL, TOTAL</strong> (Saleable Production) → <strong>Billets, BARS, PLATES, Saleable Steel</strong>, and <strong>TOTAL</strong> (the separate Despatch section) → <strong>Saleable Steel Despatch</strong> — for both the report month and the previous month (its own dedicated Actual column in the same table), 16 rows total.</li>
          ...
          {bslProdAllMonths && (
            <div style={{ padding: '20px', backgroundColor: '#f8f9fa', border: '1px solid #06b6d4', borderRadius: '8px' }}>
              ...
              <div style={{ fontSize: '8pt', color: '#5f6368', marginTop: 12, padding: 10, backgroundColor: '#ffffff', borderRadius: 4 }}>
                ...
                GP Sheet + GC Sheet; <code>CR(1&amp;2) Total Saleable</code> / <code>CR III Total Saleable</code> are also computed here. Legacy items <code>CRC&amp;S(1&amp;2)</code> / <code>CRC(3)</code> are never written by this route.
              </div>
            </div>
          )}
