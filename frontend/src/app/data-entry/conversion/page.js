        ...
        <div style={{ backgroundColor: '#fff', border: '1px solid #dadce0', borderRadius: '8px', overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', backgroundColor: '#e8f0fe', color: '#174ea6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              <span style={{ fontSize: '9pt', color: '#174ea6' }}>FY:</span>
              <select value={fyStart} onChange={e => setFyStart(e.target.value)}
                style={{ fontSize: '9pt', padding: '2px 6px', borderRadius: '4px', border: '1px solid #dadce0', backgroundColor: '#fff', color: '#202124' }}>
          ...
          {loading ? (
            <div style={{ padding: '24px', textAlign: 'center', color: '#5f6368', fontSize: '10pt' }}>Loading...</div>
          ) : (
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '9.5pt' }}>
                <thead>
                  <tr style={{ backgroundColor: '#f8f9fa' }}>
                    <th style={{ padding: '8px 16px', textAlign: 'left', fontWeight: '700', color: '#5f6368', borderBottom: '1px solid #dadce0', width: '50%' }}>Month</th>
                    <th style={{ padding: '8px 12px', textAlign: 'center', fontWeight: '700', color: '#188038', borderBottom: '1px solid #dadce0' }}>Actual ('000 T)</th>
                  </tr>
                </thead>
                <tbody>
                  {fyMonths.map((m, idx) => {
                    ...
                    return (
                      <tr key={m} style={{ backgroundColor: isEdited ? '#fef7e0' : idx % 2 === 0 ? '#fff' : '#f8f9fa', borderBottom: '1px solid #f1f3f4' }}>
                        <td style={{ padding: '6px 16px', color: '#202124', fontWeight: '500' }}>
                          {label}
                          {isEdited && <span style={{ marginLeft: '6px', fontSize: '8pt', color: '#b06000', fontWeight: '600' }}>edited</span>}
                        </td>
                        <td style={{ padding: '6px 12px', textAlign: 'center' }}>
                          <input type="number" step="0.001" value={val}
                            onChange={e => setEdits(prev => ({ ...prev, [m]: e.target.value }))}
                            placeholder="Enter actual"
                            style={{ width: '120px', padding: '4px 6px', border: `1px solid ${val ? '#a8dab5' : '#dadce0'}`, borderRadius: '4px', textAlign: 'right', fontSize: '9pt', color: '#188038', backgroundColor: val ? '#e6f4ea' : '#fff' }} />
                        </td>
                      </tr>
                    );
                  })}
          ...
          {status && (
            <div style={{ margin: '0 16px 12px', padding: '8px 12px', borderRadius: '6px', fontSize: '0.8rem', backgroundColor: status.type === 'success' ? '#e6f4ea' : '#fce8e6', color: status.type === 'success' ? '#188038' : '#d93025' }}>
              {status.text}
            </div>
          )}

          <div style={{ padding: '12px 20px', backgroundColor: '#f8f9fa', borderTop: '1px solid #dadce0', display: 'flex', justifyContent: 'flex-end' }}>
            <button onClick={handleSave} disabled={saving || !hasEdits}
              style={{ padding: '6px 16px', fontSize: '9pt', backgroundColor: hasEdits ? '#10b981' : '#5f6368', color: '#fff', border: 'none', borderRadius: '4px', cursor: hasEdits ? 'pointer' : 'default', fontWeight: '600' }}>
