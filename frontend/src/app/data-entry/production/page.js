      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '40px 32px', display: 'flex', flexDirection: 'column' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto', width: '100%', flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ marginBottom: '32px', flexShrink: 0 }}>
            <h1 style={{ fontSize: '28pt', fontWeight: '800', color: '#202124', margin: '0 0 4px 0' }}>
              📊 Production Data Entry
            </h1>
            <p style={{ fontSize: '14pt', color: '#5f6368', margin: '0 0 16px 0' }}>
            ...
            <div style={{
              ...
              <div>
                <label style={{ fontSize: '13pt', fontWeight: '600', color: '#5f6368', display: 'block', marginBottom: '6px' }}>Plant</label>
                <select
                  value={plant}
                  onChange={e => { setPlant(e.target.value); setLoaded(false); setItems([]); }}
                  style={{ padding: '10px 14px', fontSize: '13pt', width: '100%', borderRadius: '4px', border: '1px solid #dadce0' }}
                >
              ...
              <div>
                <label style={{ fontSize: '13pt', fontWeight: '600', color: '#5f6368', display: 'block', marginBottom: '6px' }}>Month</label>
                <select
                  value={month}
              ...
              <div>
                <label style={{ fontSize: '13pt', fontWeight: '600', color: '#5f6368', display: 'block', marginBottom: '6px' }}>Year</label>
                <select
                  value={year}
            ...
            {loaded && items.length > 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                <div style={{ flex: 1, overflowX: 'auto', overflowY: 'auto', marginBottom: '16px', border: '1px solid #dadce0', borderRadius: '6px' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: '#fff' }}>
                    <thead style={{ position: 'sticky', top: 0, zIndex: 10 }}>
                      <tr style={{ backgroundColor: '#f8f9fa', borderBottom: '1px solid #dadce0' }}>
                        <th style={{ padding: '12px 14px', textAlign: 'left', fontWeight: '700', fontSize: '12pt', color: '#5f6368' }}>Item</th>
                        <th style={{ padding: '12px 14px', textAlign: 'right', fontWeight: '700', fontSize: '12pt', color: '#5f6368' }}>Plan ('000T)</th>
                        <th style={{ padding: '12px 14px', textAlign: 'right', fontWeight: '700', fontSize: '12pt', color: '#5f6368' }}>Actual ('000T)</th>
                        <th style={{ padding: '12px 14px', textAlign: 'center', fontWeight: '700', fontSize: '12pt', color: '#5f6368' }}>Variance</th>
                      </tr>
                    </thead>
                    <tbody>
                      {items.map((item, idx) => {
                          ...
                          <tr key={idx} style={{ borderBottom: '1px solid #f8f9fa' }}>
                            ...
                            <td style={{ padding: '12px 14px', textAlign: 'right' }}>
                              <input
                                ...
                                style={{ width: '120px', padding: '8px 10px', border: '1px solid #dadce0', borderRadius: '4px', textAlign: 'right', fontSize: '12pt' }}
                              />
                            </td>
                            <td style={{ padding: '12px 14px', textAlign: 'right' }}>
                              <input
                ...
                <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', marginTop: 'auto', paddingTop: '16px' }}>
                  ...
                  <button
                    ...
                    style={{ padding: '10px 20px', borderRadius: '4px', border: 'none', backgroundColor: hasChanges ? '#10b981' : '#5f6368', color: '#fff', fontSize: '12pt', fontWeight: '600', cursor: hasChanges ? 'pointer' : 'default' }}
                  >
            ...
            {!loaded && !loading && (
              <div style={{ padding: '50px', textAlign: 'center', color: '#5f6368', fontSize: '13pt', backgroundColor: '#fff', borderRadius: '6px', border: '1px solid #dadce0', flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
